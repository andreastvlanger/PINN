...
boundary_mask_np = (X == rx) | (X==lx) | (Y == ly) | (Y == uy) 
bc_mask = boundary_mask_np.flatten().astype(np.float32)

# Convert all points and boundary mask to tensors
all_points_tf = tf.convert_to_tensor(domain_points, dtype=tf.float32)
boundary_mask_tf = tf.convert_to_tensor(bc_mask, dtype=tf.float32)
...
u2D = np.reshape(best_solution,(Nx, Nx))
# Plot the heatmap   
plt.figure()
...
u_exact = PoissonFDM_Rec.poisson_exact(x, y, N=501)  
