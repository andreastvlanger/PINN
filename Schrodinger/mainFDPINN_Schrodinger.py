...
bc = True # If False boundary conditions are not included in Neural Networks
params['bc'] = bc
...
loss_fc_data['lambda_boundary'] = lambda_boundary
...
H_pred = np.reshape(h_pred, (Nt, Nx-1))
plt.figure()
plt.imshow(H_pred.T, 
           aspect='auto', 
...
try:
    ...
    for time_ in [time1,time2,time3]:
        ...
        plt.xlim([-5.1,5.1])
        plt.ylim([-0.1,5.1])
        plt.xticks([-5, 0, 5], fontsize=12)  # Set tick positions
        plt.yticks([0,1,2,3,4,5], fontsize=12)  # Set tick positions
        plt.legend()
